 ...
 */
package com.vn.appusuario.modelo.test;
...
public class TestUsuarios {
    ...
    public void obtenerListaUsuario() throws SQLException, Exception {
        ArrayList<Usuario> listaCreados = (ArrayList<Usuario>) servUser.leerTodos();

        listaCreados.add(servUser.crear("aaaa@aaaa.com", "password", "Luis", "20"));
        listaCreados.add(servUser.crear("aaab@aaab.com", "password", "Paco", "30"));
        listaCreados.add(servUser.crear("aaac@aaac.com", "password", "Ana", "50"));
        
        ArrayList<Usuario> listaTotal = (ArrayList<Usuario>) servUser.leerTodos();
        
        for(Usuario usuario1:listaCreados){
            boolean encontrado = false;
            if(usuario1 != null && listaTotal.contains(usuario1)){
                servUser.eliminar(usuario1.getId());
            }else{
                fail("No se ha anadido o no se ha encontrado el listaTotal");
            }
        }
    }
